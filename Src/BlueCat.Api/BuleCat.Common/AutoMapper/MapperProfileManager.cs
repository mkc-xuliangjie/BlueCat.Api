...
namespace BuleCat.Common.AutoMapper
{
    /// <summary>
    /// Mapper 映射关系管理器
    /// </summary>
    public class MapperProfileManager
    {
        ...
        {
            var mapperTypes = types.Where(t => t.GetTypeInfo().IsDefined(typeof(MapMemberAttribute), false));
        ...
        public void Buid()
        {
            Mapper.Mapper.Initialize(mapperConfig);
        }
           ...
           where TAttribute : MapperAttribute
        {
            foreach (var mapper in type.GetCustomAttributes(typeof(TAttribute), false).OfType<MapperAttribute>())
            {
                ...
                Mapper.IMappingExpression reverMappingExpression = null;
